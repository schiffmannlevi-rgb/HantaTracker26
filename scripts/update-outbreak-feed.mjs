...
function applyParsedFeed(feed, parsed) {
  ...
  feed.meta.statusText = `${parsed.sourceLabel}: ${parsed.confirmedCases} confirmed, ${parsed.probableCases} probable, ${parsed.deaths} deaths. No synthetic counts are displayed.`;
  feed.meta.updateCadence = "Daily at 8:00 AM local; browser checks the local feed every 5 minutes.";
  feed.meta.summary = {
  ...
  if (cluster) {
    ...
    cluster.notes = `Official WHO snapshot for the cruise-linked Andes hantavirus event: ${parsed.confirmedCases} confirmed cases, ${parsed.probableCases} probable cases, and ${parsed.deaths} deaths. WHO assessed global public-health risk as ${parsed.globalRisk.toLowerCase()}.`;
  }

  const latestTimeline = feed.timeline.find((event) => event.title === "WHO official snapshot");
  if (latestTimeline) {
    latestTimeline.date = parsed.asOfDate;
    latestTimeline.description = `WHO reports ${parsed.confirmedCases} confirmed cases, ${parsed.probableCases} probable cases, and ${parsed.deaths} deaths, with ${parsed.globalRisk.toLowerCase()} global public-health risk.`;
  }
}
...
for (const source of sources) {
  ...
  parsed = source.kind === "who-briefing" ? parseWhoBriefing(text) : parseWhoDon(text);
  if (parsed) break;
}
...
console.log(
  `Updated outbreak feed from ${parsed.sourceLabel}: ${parsed.confirmedCases} confirmed, ${parsed.probableCases} probable, ${parsed.deaths} deaths as of ${parsed.asOfDate}.`
);
